...
BATTERY_LIFE_MINUTES = 6
BATTERY_LIFE_SECONDS = 30  #BATTERY_LIFE_MINUTES * 60
...
def find_shortest_path_home(current_pos, map_image):
    global PATH_HISTORY
    start_point = tuple(map(int, PATH_HISTORY[0]))
    current_pos = tuple(map(int, current_pos))

    path = astar(current_pos, start_point, map_image)
...
def display_map(image_path, drone_pos_cm):
    ...
    while True:
        ...
        '''
        we retuend to the start and now we are going to the points we put along the map 
        '''
        # if returnd_to_start_flag:
        #     # we go to the first point
        #     if drone_pos_px[0] == POINT_HISTORY[POINT_HISTORY_INDEX].get_x() and drone_pos_px[1] == POINT_HISTORY[POINT_HISTORY_INDEX].get_y():
        #         # we arrived
        #         returnd_to_start_flag = False
        #         if POINT_HISTORY[POINT_HISTORY_INDEX].get_inf_left():
        #             following_wall_direction = [-1, 0]
        #             main_movment_direction = [-1, 0]
        ...
        #         drone_pos_px = PATH_HISTORY[RETURNED_INDEX]
        ...
        if return_journey:
            ...
            flag2 = False
            false_corner = False
            if return_index == len(PATH_HISTORY) - 1:  # First step of return journey
                current_pos = tuple(map(int, drone_pos_px))
                shortest_path = find_shortest_path_home(current_pos, map_image)
                if len(Return_path) > 0:
                    Return_path.clear()
                Return_path = shortest_path
                return_index = 0
                # print(f"Calculated shortest path home: {PATH_HISTORY}")
            if return_index < len(Return_path):
                prev_drone_pos = drone_pos_px
                drone_pos_px = Return_path[return_index]
                return_index += 1

            # draw_drone_detect_and_color(screen, prev_drone_pos, drone_pos_px, map_image, direction,main_movment_direction)  # Draw the drone and its detection range on the map
            ...
            #     draw_drone_detect_and_color(screen, prev_drone_pos, drone_pos_px, map_image, direction,
            #                                 main_movment_direction)  # Draw the drone and its detection range on the map
            ...
            was_here = draw_drone_detect_and_color(screen, prev_drone_pos, drone_pos_px, map_image, direction,
                                                   main_movment_direction)  # Draw the drone and its detection range on the map
            if (was_here):
                LATS_FIVE_MOVES.append("1")
            else:
                LATS_FIVE_MOVES.clear()
            # if len(LATS_FIVE_MOVES) == 5:
            # print("yes")
...
def draw_drone_detect_and_color(screen, prev_pos, drone_pos_px, map_image, movment_direction, main_movment_direction):
...
# def smart_return(desierd_location):
#     for i in len(PATH_HISTORY):
#         dist_x = abs(desierd_location[0] - PATH_HISTORY[i][0])
#         dist_y = abs(desierd_location[1] - PATH_HISTORY[i][1])
#         var = 15
#         if dist_y <= var and dist_x <= var:
#             return
'''
...
def Point_displacement(screen, center_x, center_y, inf_directions,movment_direction):  # fix do calculations in detect and color !!!!!
