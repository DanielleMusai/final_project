...
def Point_displacement(screen, center_x, center_y, inf_directions,movment_direction):  # fix do calculations in detect and color !!!!!
    ...
    for dx, dy in inf_directions:
        if (dx == 0 and dy < 0) and not ((movment_direction[0] == 0 and movment_direction[1] < 0) or (movment_direction[0] == 0 and movment_direction[1] > 0)): # if inf is up direction cant be up or down
            new_point.inf_front = True
            add = True

        elif (dx == 0 and dy > 0) and not ((movment_direction[0] == 0 and movment_direction[1] > 0) or (movment_direction[0] == 0 and movment_direction[1] < 0)): # if inf is down direction cant be down or up
            new_point.inf_back = True
            add = True

        elif (dx > 0 and dy == 0) and not ((movment_direction[0] > 0 and movment_direction[1] == 0) or (movment_direction[0] < 0 and movment_direction[1] == 0)): # if inf is right direction cant be right or left
            new_point.inf_right = True
            add = True

        elif (dx < 0 and dy == 0) and not ((movment_direction[0] < 0 and movment_direction[1] == 0) or (movment_direction[0] > 0 and movment_direction[1] == 0)): # if inf is left direction cant be left or right
