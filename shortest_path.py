...
def get_neighbors(pos, map_image):
    ...
    for dx, dy in [(0, 1), (1, 0), (0, -1), (-1, 0)]:
        ...
        if 0 <= new_pos[0] < map_image.get_width() and 0 <= new_pos[1] < map_image.get_height():
            if map_image.get_at(new_pos) != BLACK:
...
def display_map(image_path, drone_pos_cm):
    global PATH_HISTORY, POINT_HISTORY, ENDLESS_LOOP
    PATH_HISTORY = []
    ...
    while True:
        ...
        if return_journey:
            if return_index == len(PATH_HISTORY) - 1:  # First step of return journey
                ...
                PATH_HISTORY = shortest_path
                return_index = 0
                print(f"Calculated shortest path home: {PATH_HISTORY}")

            if return_index < len(PATH_HISTORY):
                prev_drone_pos = drone_pos_px
                drone_pos_px = PATH_HISTORY[return_index]
                return_index += 1

                screen.blit(map_image, (0, 0))
                draw_drone_detect_and_color(screen, prev_drone_pos, drone_pos_px, map_image)
                ...
                pygame.quit()
                sys.exit()  # This will close the program
            ...
            was_here = draw_drone_detect_and_color(screen, prev_drone_pos, drone_pos_px, map_image,
...
def draw_drone_detect_and_color(screen, prev_pos, drone_pos_px, map_image, movment_direction=None):
...
if __name__ == "__main__":
    ...
    image_path = "C:\\Users\\user\\PycharmProjects\\final_project\\Maps\\p11.png"
